...
ruleViewer <- function(rules, viz, class){
      ...
      result = switch( viz,  
            ...
            "df"= choix,
            "bar"=       ggplot(freq[1:15,], aes(x= reorder(RHS, -n), y = log10(n) )) + geom_col(data = freq, aes(x = reorder(RHS, -n), y = log10(n), fill = MeSH_Ancestry)) +geom_point(pch = 21, aes(x = reorder(RHS, -n), y = log10(n), fill = MeSH_Ancestry)) + ggtitle(paste("Rule frequencies based on:", toupper(class))) + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
                                                                                                                                                                                                                                                                                              panel.background = element_blank(), axis.line = element_line(colour = "black")) +
                  ylab("log10(number of Keyword Reltionships)") + xlab("RHS of Keyword Relationship")+ theme(axis.text.y = element_text(size = 15)) + theme(axis.title = element_text(size = 16)) + theme(title = element_text(size = 20)) + theme(legend.position = c(0.85,0.85)) +
